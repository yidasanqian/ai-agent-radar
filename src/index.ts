/**
 * AI Agent Radar: daily digest for AI CLI tools and agent ecosystem projects.
 *
 * Env vars:
...
async function main(): Promise<void> {
  ...
  if (genZh) await saveWebReport(webResults, webState, utcStr, dateStr, footer, "zh");
  if (genEn) await saveWebReport(webResults, webState, utcStr, dateStr, enFooter, "en");

  await Promise.all([
    genZh && zhSummaries
      ? saveTrendingReport(trendingData, zhSummaries.trendingSummary, utcStr, dateStr, footer, "zh")
      : Promise.resolve(),
    genEn && enSummaries
      ? saveTrendingReport(trendingData, enSummaries.trendingSummary, utcStr, dateStr, enFooter, "en")
      : Promise.resolve(),
    genZh ? saveHnReport(hnData, utcStr, dateStr, footer, "zh") : Promise.resolve(),
    genEn ? saveHnReport(hnData, utcStr, dateStr, enFooter, "en") : Promise.resolve(),
  ]);
